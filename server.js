...
io.on('connection', (socket) => {
    ...
    socket.on('admin:resetImpact', ({ teamId }) => {
        ...
        if (team) {
            // Case 1: They have it active currently (haven't bought/lost yet) -> remove money, reset flags
            if (team.impactActive) {
                team.purse = Number(team.purse) - bonus;
            }
            // Case 2: They used it and lost (so money was already removed), but we want to let them use it again.
            // No purse action needed here unless you want to refund a lost impact? 
            // Standard logic: Resetting allows them to trigger it again.
    ...
    socket.on('bid:request', (data) => {
        io.emit('admin:toast', { msg: `✋ Bid Req: ${data.teamName} for ${data.playerName}` });
    });
    ...
    socket.on('player:sold', (data) => {
        ...
        if (team) {
            ...
            // Check Impact Logic for ALL teams
            STATE.teams.forEach(t => {
                if (t.impactActive) {
                    if(t.impactTarget === soldKey) {
                        if(t.id === data.teamId) {
                            // Winner: They keep the bonus money (used for purchase)
                            t.impactActive = false; 
                        } else {
                            // Loser: Money is deducted immediately
